...
export async function executePlan(
  ...
  for (let i = 0; i < actionsToExecute.length; i++) {
    ...
    console.log(`[EXECUTE] [${i + 1}/${actionsToExecute.length}] ${action.from} -> ${action.to}`);
...
export async function executeRollback(
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    console.log(`[ROLLBACK] [${i + 1}/${entries.length}] ${entry.from} -> ${entry.to}`);
...
export async function saveExecutionLog(
  ...
  for (const result of report.results) {
    ...
    lines.push(`${statusSymbol} [${result.status.toUpperCase()}] ${action.actionType}`);
