    ...
    for (const dirent of dirEntries) {
      ...
      if (dirent.isDirectory()) {
        ...
        if (isGeneratedFolder(dirent.name)) {
          const entry = await createGeneratedFolderEntry(
            fullPath,
            relativePath
          );
          entries.push(entry);
...
async function classifyFile(
  ...
  if (
    ...
    [".jpg", ".jpeg", ".png", ".gif", ".heic", ".webp"].includes(
      entry.extension
    )
  ) {
    entry.type = "Media";
    ...
    [".js", ".ts", ".py", ".java", ".cpp", ".c", ".h"].includes(entry.extension)
  ) {
    entry.type = "Code";
...
async function classifyDocumentWithAI(
  ...
  if (!options.ollamaModel && !options.useAI) {
    return;
  }
