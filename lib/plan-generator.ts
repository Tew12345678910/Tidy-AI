...
export async function generatePlan(
  ...
  for (const entry of manifest.entries) {
    const action = await createAction(entry, destRoot, preferences, projectRoots, manifest);
    if (action) {
      actions.push(action);
    }
  }
...
async function createAction(
  ...
  const approved = entry.confidence >= preferences.confidenceThresholds.autoApprove;
...
function determinDestinationPath(
  ...
  if (entry.type === 'Document' && entry.metadata) {
    ...
    const subject = entry.suggestedCategory || entry.metadata.subject || 'Documents';
 ...
 */
function buildActionReason(entry: ManifestEntry, actionType: ActionType): string {
 ...
 */
async function resolveCollisions(actions: PlanAction[], destRoot: string): Promise<void> {
  ...
  for (const [dest, conflictingActions] of destinationMap) {
    if (conflictingActions.length > 1) {
      ...
      console.log(`[PLAN] Collision detected at ${dest}: ${conflictingActions.length} files`);
...
function performSafetyCheck(
  ...
  for (const action of actions) {
    ...
    if (action.movesInsideProjectRoot) {
      ...
      errors.push(`Action ${action.id} attempts to move file inside project root`);
    }
    
    // Check for low confidence
    if (action.confidence < 0.5 && action.actionType !== 'skip') {
      lowConfidenceActions++;
      warnings.push(`Action ${action.id} has low confidence (${action.confidence})`);
    }
 ...
 */
export async function saveRollback(rollback: Rollback, outputDir: string): Promise<string> {
  await fs.mkdir(outputDir, { recursive: true });
