...
import { PROJECT_ROOT_SIGNALS, GENERATED_FOLDERS, ProjectRootDetection } from './types';

/**
 * Check if a directory is a project root
 */
export async function detectProjectRoot(dirPath: string): Promise<ProjectRootDetection> {
 ...
 */
function inferProjectType(signals: string[]): ProjectRootDetection['projectType'] {
  ...
  // Python
  if (signalSet.has('pyproject.toml') || 
...
export function isInsideProjectRoot(
  ...
  for (const [projectRootPath] of projectRoots) {
    if (filePath.startsWith(projectRootPath + path.sep) || filePath === projectRootPath) {
...
export function describeProject(detection: ProjectRootDetection): string {
  ...
  return `Project${type}: ${signals}${detection.signals.length > 3 ? '...' : ''}`;
}
